...
#' Exports a CV plan as tibble. Useful if a package is not integrated
#' into the modeltime framework or for usage in Python.
#'
#' @param cv_plan A cross validation plan generated by ```modeltime.resample::time_series_cv```
#'
#' @return A tibble containing the cross validation plan. The id columns stores the Slice information. The dataset contains the information if it is the train part (dataset = "train") or if it is the part which should be forecasted (dataset = "validation").
#' @export
#'
#' @examples
export_cv_to_tibble <- function(cv_plan) {

  validation_data <- tibble::tibble()

  for (current_split_id in cv_plan$id) {

    current_split <- cv_plan %>%
      dplyr::filter(id == current_split_id)

    current_split_data_train <- current_split$splits[[1]] %>%
      rsample::training() %>%
      dplyr::mutate(dataset = "train")

    current_split_data_val <- current_split$splits[[1]] %>%
      rsample::assessment() %>%
      dplyr::mutate(dataset = "validation")

    current_split_data <- current_split_data_train %>%
      dplyr::bind_rows(current_split_data_val) %>%
      dplyr::mutate(id = current_split_id)

    validation_data <- validation_data %>%
      dplyr::bind_rows(current_split_data)
  }
